...
import { SEOIntelligence } from './seo/SEOIntelligence';

export function SEOAdminTab() {
  const [activeSubTab, setActiveSubTab] = useState<'monitoring' | 'intelligence' | 'config'>('intelligence');
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-500">Monitoramento de custos, uso de APIs e inteligência competitiva.</p>
        </div>
        
        <div className="flex bg-gray-100 p-1 rounded-xl">
          <button
            onClick={() => setActiveSubTab('monitoring')}
            className={`px-4 py-2 text-sm font-bold rounded-lg transition-all ${activeSubTab === 'monitoring' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-900'}`}
          >
            Monitoramento
          </button>
          <button
            onClick={() => setActiveSubTab('intelligence')}
            className={`px-4 py-2 text-sm font-bold rounded-lg transition-all ${activeSubTab === 'intelligence' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-900'}`}
          >
            Inteligência Competitiva
          </button>
          <button
            onClick={() => setActiveSubTab('config')}
            className={`px-4 py-2 text-sm font-bold rounded-lg transition-all ${activeSubTab === 'config' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-900'}`}
          >
            Configurações
          </button>
        </div>
      ...
      {activeSubTab === 'intelligence' && <SEOIntelligence />}
      
      {activeSubTab === 'config' && <SEOConfig />}
